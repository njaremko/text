 ...
 *   h.init()
 *
 *   fmt.Println(strings.Join(h.hypthnateWord("associate"), "-"))
 *   fmt.Println(strings.Join(h.hypthnateWord("exceptional"), "-"))
 *   fmt.Println(strings.Join(h.hypthnateWord("accoutrement"), "-"))
 *   fmt.Println(strings.Join(h.hypthnateWord("circumlocution"), "-"))
 *   fmt.Println(strings.Join(h.hypthnateWord("perfidiousness"), "-"))
 */

var cachePath = filepath.Join(os.ExpandEnv("$GOPATH"), "/src/github.com/njaremko/text/cachedPatterns")
...
// Type that allows for recursive map of maps that eventually
// point to an []int
type recursiveMap map[string]interface{}

type Hyphenator struct {
	// Hyphenation masks for exceptions to general rule
	Exceptions map[string][]int
	// General tree that implements Liang's algorithm
	Tree recursiveMap
}

// cache serialized Hyphenator into a gob for future use
func (h Hyphenator) cache() {
	if _, err := os.Stat(cachePath); os.IsNotExist(err) {
		gob.Register(recursiveMap{})
		gob.Register(h)
		stored := bytes.Buffer{}
		encoder := gob.NewEncoder(&stored)
		if err := encoder.Encode(h); err == nil {
			if err = ioutil.WriteFile(cachePath, stored.Bytes(), 0644); err != nil {
				log.Println("Couldn't", err)
			}
		} else {
			log.Println("Couldn't encode Hyphenator: ", err)
		}
	}
}

// init is used to take string of patterns, and put them in the tree
func (h *Hyphenator) init() {

	// If we've cached the Hyphenator already, just load it
	if _, err := os.Stat(cachePath); err == nil {
		...
		if data, err = ioutil.ReadFile(cachePath); err != nil {
			log.Println("Couln't read file: ", cachePath)
	...
	// Remove newlines, otherwise end of line pattern won't be loaded
	patterns = strings.Replace(patterns, "\n", " ", -1)
	exceptions = strings.Replace(exceptions, "\n", " ", -1)
	// Initialize h.tree to prevent errors
	h.Tree = recursiveMap{}
	// Load general patterns into tree
	for _, pattern := range strings.Split(patterns, " ") {
		h.insertPattern(pattern)
	}
	// Create exception map
	h.Exceptions = map[string][]int{}
	regex := regexp.MustCompile("[a-z]")
	for _, ex := range strings.Split(exceptions, " ") {
		word := strings.Replace(ex, "-", "", -1)
		split := regex.Split(ex, -1)
		h.Exceptions[word] = make([]int, len(split)+1)
		for i, s := range split {
			if s == "-" {
				h.Exceptions[word][i+1] = 1
			} else {
				h.Exceptions[word][i+1] = 0
			}
		}
	}
	// Cache the tree so future hyphenations are much faster
	h.cache()
}

// insertPattern parses a pattern and stores the mask
// into the tree
func (h *Hyphenator) insertPattern(pattern string) {
	charsRegex := regexp.MustCompile("[0-9]")
	pointsRegex := regexp.MustCompile("[.a-z]")
	chars := charsRegex.ReplaceAllString(pattern, "")
	split := pointsRegex.Split(pattern, -1)
	points := make([]int, len(split))
	for i, d := range split {
...
// hyphenateWord takes a string, and returns a []string
// broken where hyphenation is possible
func (h Hyphenator) hypthnateWord(word string) []string {
...
// These are exceptions to the general rules.
var exceptions = `as-so-ciate as-so-ciates dec-li-na-tion oblig-a-tory phil-an-thropic present
